# from https://github.com/shibukawa/imagesize_py/blob/master/imagesize.py
import io
import os
import re
import struct
...
def convertToDPI(density, unit):
    ...
    elif unit == UNIT_0_1MM:
        return density * 254
    elif unit == UNIT_0_01MM:
        return density * 2540
    elif unit == UNIT_UM:
            ...
            while True:
                ...
                if chunkType == b'pHYs':
                    try:
                        ...
                        raise ValueError("Invalid PNG file")
                    if unit:
                        xDPI = convertToDPI(xDensity, UNIT_1M)
                        yDPI = convertToDPI(yDensity, UNIT_1M)
                    else:  # no unit
                        xDPI = xDensity
                        yDPI = yDensity
                    break
                ...
                while not 0xc0 <= ftype <= 0xcf:
                    if ftype == 0xe0:  # APP0 marker
                        ...
                        if unit == 1 or unit == 0:
                            ...
                            xDPI = convertToDPI(xDensity, UNIT_CM)
                            yDPI = convertToDPI(yDensity, UNIT_CM)
                        break
                    fhandle.seek(size, 1)
                    byte = fhandle.read(1)
                    while ord(byte) == 0xff:
                ...
                if foundResBox:
                    while headerSize > 0:
                        ...
                        if boxType == b'resd':  # Display resolution box
                            ...
                            xDPI = convertToDPI(xDensity, xUnit)
                            yDPI = convertToDPI(yDensity, yUnit)
                            break
                        boxSize, = struct.unpack('>L', boxHeader[:4])
                        fhandle.seek(boxSize - 8, 1)
                        headerSize -= boxSize
